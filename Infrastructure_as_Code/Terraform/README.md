...
Заранее оговорюсь:
В коде уже будет добавлен public key хоста, с которого мы будем осуществлять первое подключение к созданным VM. Сгенерируем на удалённом хосте private and public ssh key:

- ssh-keygen -t rsa

Полученный public key мы добавим в код terraform, для последующего подключения к нашим VM. Private key мы передадим Ansible серверу, для дальнейшей настройки инфраструктуры.
...
- disk_size
- template_name
9. Используя написанный модуль, мы развернём 6 VM на Proxmox для последующей настройки инфраструктуры. В файле main.tf, в корне.
...
Теперь наш Ansible сервер может подключаться к развёрнутым серверам.
