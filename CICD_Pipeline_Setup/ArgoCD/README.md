...
После создания, AegoCD должен развернуть полы и сервис в нашем кластере.
Описание файлов:
├── HelmCharts			#Каталог с HelmChart приложением
│   └── MyChart1			#HelmChart приложение
│       ├── Chart.yaml			#Описание, методанные, версии и тд.
...
└── prod				#Окружение prod для Argocd
    ├── applications
    │   └── app1.yaml			#Тут расписано в каком кластере будет создано, 
                                                                       из каких Chart и какой контейнер использовать
    └── root.yaml	#Создание в текущем кластере приложения из yaml файлов которые находятся в каталоге prod/applications
